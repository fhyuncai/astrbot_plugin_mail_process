...
| `/mail_confirm <请求ID>` | 确认发送 AI 代写的邮件 |
| `/mail_reject <请求ID>` | 取消发送 AI 代写的邮件 |
...
1. AI 生成待发送内容
2. 插件下发请求 ID
3. 管理员执行 `/mail_confirm <请求ID>` 后才真正通过 SMTP 发出
...
- 自动处理新邮件时，确认 `ai_allow_reply` 已开启，且目标账户已配置 SMTP
- 用户主动要求 AI 发送邮件时，需要执行 `/mail_confirm <请求ID>`
- 确认目标邮箱已配置 SMTP
